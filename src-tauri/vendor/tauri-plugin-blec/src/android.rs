...
use crate::channel::send_channel_data;
...
impl btleplug::api::Central for Adapter {
    ...
    async fn events(&self) -> Result<Pin<Box<dyn Stream<Item = CentralEvent> + Send>>> {
        ...
        let channel: Channel = Channel::new(move |response| {
            ...
            if !send_channel_data(&tx, event) {
                debug!("event receiver closed; dropping late event");
            }
            Ok(())
        });
        get_handle()
            ...
            match response.deserialize::<Notification>() {
                Ok(notification) => {
                    if !send_channel_data(
                        ...
                        debug!("notification receiver closed; dropping late notification");
                    }
                }
                Err(e) => {
